...
namespace WildBoar.GUIModule
{
	...
	public class UICameraTool : EditorWindow
	{
		...
		void OnGUI()
		{
			...
			System.Collections.Generic.List<Camera> list = NGUIEditorTools.FindAll<Camera>();
		...
		void DrawRow(Camera cam)
		{
			...
			if (cam != null)
			{
				...
				GUILayout.BeginHorizontal(GUI.skin.textArea, GUILayout.MinHeight(20f));
			...
			if (cam != null)
			{
				...
				{
					cam.enabled = !enabled;
					NGUITools.EditorSetDirty(cam.gameObject);
				}
			}
			else
			{
			...
			{
				EventSystem uic = cam.GetComponent<EventSystem>();
				bool ev = (uic != null && uic.enabled);

				if (ev != EditorGUILayout.Toggle(ev, GUILayout.Width(20f)))
				{
					if (uic == null) uic = cam.gameObject.AddComponent<EventSystem>();
					uic.enabled = !ev;
				}
			}
			...
			else
			{
				...
				{
					NGUIEditorTools.UndoRecordObject("Camera Mask Change", cam);
