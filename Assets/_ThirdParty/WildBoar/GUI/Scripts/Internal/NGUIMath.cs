	...
	static public class NGUIMath
	{
		...
		static public Rect ConvertToPixels(Rect rect, int width, int height, bool round)
		{
			...
			if (round)
			{
				...
				final.yMin = Mathf.RoundToInt(rect.yMin * height);
				final.yMax = Mathf.RoundToInt(rect.yMax * height);
			}
			else
			{
				final.xMin = rect.xMin * width;
		...
		[System.NonSerialized] static List<UIDrawCallMaker> s_widgets = new List<UIDrawCallMaker>();
		...
		static public Bounds CalculateAbsoluteWidgetBounds(Transform trans)
		{
			if (trans != null)
			{
				...
				{
					var w = s_widgets[i];
					var corners = w.RenderTargetWorldCorners;

					foreach (var corner in corners)
					{
						if (corner.x > vMax.x) vMax.x = corner.x;
						if (corner.y > vMax.y) vMax.y = corner.y;
						if (corner.z > vMax.z) vMax.z = corner.z;

						if (corner.x < vMin.x) vMin.x = corner.x;
						if (corner.y < vMin.y) vMin.y = corner.y;
						if (corner.z < vMin.z) vMin.z = corner.z;
					}
				}
			...
			ref Matrix4x4 toLocal, ref Vector3 vMin, ref Vector3 vMax, ref bool isSet, bool considerChildren)
		{
...

			{
				...
				var w = content.GetComponent<UIDrawCallMaker>();
				...
				if (w != null && w.enabled )
				{
					...
					foreach (var corner in corners)
					{
						var v = toLocal.MultiplyPoint3x4(corner);
